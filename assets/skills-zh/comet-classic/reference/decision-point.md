...
## Open / Design / Build 确认边界

保留 Open 产物审视、Design 方案确认、Build 执行配置三个确认点，各自确认不同的交付内容。初始澄清只收集关键缺口；包装格式等不阻塞方案形成且可同时决定的细节并入对应确认，不提前单独询问。已确认且仍有效的选择在摘要中回显；发现新证据或范围、风险变化时说明影响，再只确认受影响事项。推荐值和待确认假设不能作为授权，暂停后的继续意图仍需明确。
