...
- Classic Open / Native Shape: after requirements are clear and formal requirement/spec artifacts exist, bind delivery. Check the entire scope of an existing issue, or create an issue when authorized.
- Classic Design / Native requirement revision: synchronize scope. Preserve existing AC keys; omit the key for new criteria to allocate one. Do not delete old criteria; mark removed goals retired and provide a new scope confirmation reference.
...
- Review: use a separate review execution and record its identity and evidence. A formal PR requires review even when Classic/hotfix review_mode is off. Do not invoke external Skills without the user's permission.
...
After failed creation, use observe before any retry. Do not repeat archive/commit or directly run gh create again. Closed and merged PRs are still existing remote results. If the outcome remains uncertain, report the blocker and continue read-only reconciliation; an empty query is not permission to create again. Report remote SHA drift instead of overwriting it during recovery.
