...
export function grant(record: DeliveryRecord, value: unknown): void {
  ...
  requireCondition(
    ['issue:create', 'issue:update', 'push', 'pull-request:create', 'pull-request:update'].includes(
      String(input.action),
    ),
  ...
  if (action === 'pull-request:update')
    requireCondition(record.pr !== null, 'Bind a pull request before granting update');
  record.grants.push({
    action,
    ...
    prNumber: action === 'pull-request:update' ? record.pr!.number : null,
  });
}
  ...
  requireCondition(
    record.grants.some(
      (g) =>
        ...
        (action !== 'pull-request:update' || g.prNumber === record.pr?.number),
    ),
